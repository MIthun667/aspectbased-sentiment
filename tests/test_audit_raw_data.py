...
from scripts.audit_raw_data import audit_file
...
def test_valid_record_has_no_errors(tmp_path: Path) -> None:
...
def test_detects_target_span_mismatch(tmp_path: Path) -> None:
    record = valid_record()
...
def test_detects_misaligned_pos_tags(tmp_path: Path) -> None:
    record = valid_record()
    record["pos"] = ["DT"]
...
def test_detects_invalid_short_matrix(tmp_path: Path) -> None:
    record = valid_record()
    record["short"][0][1] = 9
