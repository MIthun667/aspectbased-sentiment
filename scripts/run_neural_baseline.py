...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train and evaluate a reproducible neural "
            "aspect-sentiment baseline."
        )
    )
    ...
    parser.add_argument(
        "config",
        type=Path,
        help="Path to the neural experiment YAML file.",
    )
...
def class_weights_from_records(
    ...
    *,
    ...
    if np.any(counts <= 0):
        raise ValueError(
            "All classes must appear in training data "
            "when class weighting is enabled"
        )
...
def run_experiment(
    ...
    *,
    ...
    if config.model.name != "target_aware_bilstm":
        raise ValueError(
            "run_neural_baseline.py supports only "
            "model.name='target_aware_bilstm'"
        )
    ...
    try:
        ...
        validation_result = evaluate_neural_model(
            model=model,
            data_loader=validation_loader,
            loss_function=loss_function,
            device=device,
        ...
        for domain in config.data.evaluation_domains:
            ...
            test_result = evaluate_neural_model(
                model=model,
