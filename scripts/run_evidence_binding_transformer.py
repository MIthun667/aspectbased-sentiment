...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train and evaluate the evidence-binding "
            "DeBERTa classifier."
        )
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Path to an evidence-binding experiment "
            "configuration."
        ),
    )
...
def run_experiment(
    ...
    *,
    ...
    if config.model.name != (
        ...
        raise ValueError(
            "run_evidence_binding_transformer.py "
    ...
    if parameter_dtypes != {
        ...
        raise TypeError(
            "Binding model parameters are not "
    ...
    try:
        for epoch in range(
            ...
            print(
                ...
                f"| Train Combined "
                f"{training_result.combined_loss:.4f} "
                f"| Train Evidence "
                f"{training_result.evidence_loss:.4f} "
                f"| Train Agreement "
                f"{training_result.agreement_loss:.4f} "
                f"| Val Combined F1 "
                f"{metric_value:.4f} "
                f"| Val Context F1 "
