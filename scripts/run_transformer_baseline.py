...
from src.aspect_sentiment.data import load_canonical_split
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "transformer aspect-sentiment baseline."
        )
    )
    ...
    parser.add_argument(
        "config",
        type=Path,
        help="Path to the transformer experiment YAML file.",
    )

    return parser.parse_args()


def create_transformer_data_loader(
...
def run_experiment(
    ...
    *,
    ...
    if config.model.name != "deberta_pair_classifier":
        raise ValueError(
            "run_transformer_baseline.py supports only "
            "model.name='deberta_pair_classifier'"
        )
    ...
    train_loader = create_transformer_data_loader(
        train_records,
        tokenizer=tokenizer,
        maximum_length=maximum_length,
    ...
    validation_loader = (
        create_transformer_data_loader(
            validation_records,
            tokenizer=tokenizer,
            maximum_length=maximum_length,
            batch_size=(
